...
function PendingCandidatesList() {
  ...
  const data = api.candidate.getAllPendingCandidates.useQuery({
    searchQuery: debouncedSearch,
    filterData: selectedData,
  });
  const utils = api.useUtils();
  const updateCandidateStatus = api.candidate.updateCandidateStatus.useMutation(
    {
      onSuccess: async () => {
        await utils.candidate.getAllPendingCandidates.invalidate();
        await utils.candidate.getAllApprovedCandidates.invalidate();
        await utils.candidate.getAllRejectedCandidates.invalidate();
  ...
  return (
    <div>
      <div className="flex items-end justify-between">
        <h1 className="text-2xl font-bold">Pending Candidates</h1>
                ...
                <TableRow key={i}>
                  ...
                  <TableCell className="text-right">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      className="rotate-45 cursor-pointer text-red-600"
                      onClick={() => handleReject(candidate.id)}
                    >
                      {rejectLoading === candidate.id ? (
                        <Loader2 className="size-3 animate-spin" />
                      ) : (
